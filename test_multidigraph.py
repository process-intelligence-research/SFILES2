...
1.  Rocha Azevedo, Antonio; Nabil, Tahar; Loubière, Valentin; Privat, Romain; 
...
2.  Hirtreiter, E., Schulze Balhorn, L., & Schweidtmann, A. M. (2023). 
...
if __name__ == "__main__":
    ...
    # Dataset 2:
    dataset_1_path = current_file/"data"/"RochaAzevedo_et_al"
    csv_files = [
        dataset_1_path/"Machine Learning"/f for f in os.listdir(dataset_1_path/"Machine Learning")
        if f.endswith(".csv")
    ]
    csv_files.extend([
        dataset_1_path/"Evolutionary Programming"/f for f in os.listdir(dataset_1_path/"Evolutionary Programming")
        if f.endswith(".csv")
    ])
    csv_files.extend([
        dataset_1_path/f for f in os.listdir(dataset_1_path)
        if f.endswith(".csv")
    ])
    ...
    for file_path in csv_files:
        ...
        for col in sfiles_cols:
            ...
            flowsheet_list = [line.split(",")[col_idx] for line in lines[1:]] # lines[1:]: ignoring header
    ...
    # Dataset 2:
    dataset_2_path = current_file/"data"/"Training Data Public Upload"
    json_files = [
        f for f in os.listdir(dataset_2_path)
        if f.endswith(".json")
    ]
        
    for file_name in json_files:
        with open(dataset_2_path/file_name, "r") as f:
            try:

...
class Flowsheet:
    ...
    Attributes:
        ...
        OntoCapeConformity (bool): Specify as True when using OntoCape vocabulary (Should be the standard in future),
        defaults to False
    ...
    def convert_to_sfiles(self, version='v2', remove_hex_tags=True):
        ...
        OntoCape_mapping (bool): Specify as False if the input graph is according to SFILES standards (abbreviations,
        heat integration conventions in graph), defaults to True
    ...
    def create_random_flowsheet(self, add_sfiles=True):
        """This methods creates a random flowsheet
            ...
            in a separate class in the folder RandomFlowsheetGen
        ...
        for connection in random_flowsheet.edges:
            # adjust tags: tags:[..] to tags:{'he':[..],'col':[..]}
            # matches everything starting with 'hot', 'cold', or a number
    ...
    def map_SFILES_to_Ontocape(self, merge_HI_nodes):
        ...
        Uses the Ontocape_SFILES_mapping to modify self.state. The previous self.state is copied to the attribute
        ...
        Assume a heat exchanger that takes both top and bottom product of column -> network x cannot have 2 edges with 
        the same in and out node but different tags :( """
        if merge_HI_nodes:
            self.merge_HI_nodes()
    ...
    def map_Ontocape_to_SFILES(self):
        """
        Function that returns a graph with the node names according to SFILES abbreviations for OntoCape unit operation
        names.
        Additionally, the heat exchanger nodes (with more than 1 stream) are split up for each stream according to the
        he tags (edge attribute tags:{he:[]}).

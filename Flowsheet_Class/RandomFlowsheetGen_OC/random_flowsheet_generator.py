...
class Generate_flowsheet:
    ...
    def __init__(self):
        ...
        self.first_operation(connect_info=[])
    ...
    def raw_materials(self):
        ...
        for n in range(1, self.number_raw+1): 
            ...
            # Only one raw material
            if self.number_raw == 1: 
    ...
    def first_operation(self, connect_info=[]):
        ...
        _next_op_cat = random.choices(['Reaction', 'ThermalSeparation', 'CountercurrentSeparation', 'Filtration', 'CentrifugalSeparation'],
                                      [0.25, 0.4, 0.25, 0.05, 0.05])[0]
    ...
    def further_operation(self, connect_info=[]):
        """ The choice of further pattern/operation
            - random choice between different operation types and purification

        Parameters
        ----------
        connect_info (list): list of info that must be added to next node (e.g. previous HI or branch), default empty
        """
        # first Choice after the raw materials 
        _next_op_cat = random.choices(['Purification', 'Reaction', 'ThermalSeparation', 'CountercurrentSeparation', 'Filtration', 'CentrifugalSeparation'],
                                      [0.5, 0.125, 0.2, 0.125, 0.025, 0.025])[0]
        
        if _next_op_cat == 'Purification':
    ...
    def reaction(self, connect_info):
        ...
        # 
        pattern = random.choices(list(reactor_patterns.keys()),
                                 [i [1][0] for i in list(reactor_patterns.values())])[0]
        ...
        if 'TemperatureChange' in reactor_patterns[pattern][0]:
            special_op = random.choices(['Rec', 'HI', 'AddR', 'None'],
                                        [0.35, 0.25, 0.2, 0.2])[0]
        else:
            special_op = random.choices(['Rec', 'AddR', 'None'],
                                        [0.35, 0.2, 0.45])[0]
        ...
        if special_op == 'Rec':
            ...
            self.edges.append(((_presuc, _mixer_name), {'in_connect':_presuc_info, 'out_connect':[]})) # e.g. heat exchanger to mixer
        ...
        if special_op == 'HI':
            ...
            if '/' in _heat_ex_HI: # already heat integration
                ...
                self.edges.append(((_current_node, _next_stream_HI_node),{'in_connect':connect_info, 'out_connect':[]}))
        ...
        """ 
        if special_op == 'AddR':
            pattern = random.choices(list(addR_patterns.keys()),
                                [i [1][0] for i in list(addR_patterns.values())])[0]
            for i, _next_op_cat in enumerate(addR_patterns[pattern][0]):
                if i==0: # i=0 is the start of the incoming detergent branch -> only node no edge
                    ...
                    if _next_op_cat == 'Reaction': # no new node (already created _reac_node) and out_connect info, and reduce operation counter
                        self.edges.append(((_current_node, _reac_node), {'in_connect':connect_info, 'out_connect':[]}))
                    else: 
        ...
        #reset the last node to current_node -> Where we continue (AddR: _reac_node, HI: _next_stream_HI_node, Rec: _splitter_name)     
        self.last_node = _current_node

        # Next subprocess
        self.further_operation(connect_info)
    ...
    def separation_col(self, connect_info):
        ...
        for b in _branches:
            if b == 'bout' and random.choices(['reb', 'noreb'],[0.5, 0.5])[0] == 'reb':
                ...
                _split_op_cat = random.choices(['Splitter', 'Tank', 'Vessel'],
                                               [0.5,0.25,0.25])[0]
    ...
    def separation_sep(self, connect_info):
        ...
        - branching
        - for each branch continue to further_operation()
        """
        ...
        else: _branches=['t', 'b']  # absorption: detergent is always streaming down (higher density than gas) -> no shuffling

        """ Pattern for main inlet stream """
        if extraction: #extraction
            pattern = random.choices(list(CC_sep_patterns.keys()),
                                    [i [1][0] for i in list(CC_sep_patterns.values())])[0]
            ...
            for _next_op_cat in CC_sep_patterns[pattern][0]:
                ...
                if _next_op_cat == 'CountercurrentSeparation': 
                    ...
                    self.edges.append(((_current_node, _next_op_name), {'in_connect':connect_info, 'out_connect':['%sin'%_branches[1]]}))
                else: 
                    self.edges.append(((_current_node, _next_op_name), {'in_connect':connect_info, 'out_connect':[]}))
                connect_info=[] # reset connect_info after first edge in this function
                #reset the current node 
        ...
        for i, b in enumerate(_branches):
            if i==0: # first branch with inlet pattern for detergent
                ...
                for i, _next_op_cat in enumerate(detergent_patterns[pattern][0]):
                    ...
                    if i==0: # i=0 is the start of the incoming detergent branch -> only node no edge
                        ...
                        if _next_op_cat == 'CountercurrentSeparation': # no new node (already created _sep_node) and out_connect info
                            self.operation_counter[_next_op]-=1 # reduce operation counter
                            self.edges.append(((_current_node, _sep_node), {'in_connect':connect_info, 'out_connect':['%sin'%b]}))
                        else: 
                            # detergent preprocessing with node and edge for instance hex in Feed-hex-sep sequence 
                            self.nodes.append(_next_op_name)
                            self.edges.append(((_current_node, _next_op_name), {'in_connect':connect_info, 'out_connect':[]}))
            ...
            elif i==1: # second branch with inlet pattern for main stream (already generated in beginning of this function)
                # Connect info for the outlet branch and further operation
                connect_info=['%sout'%b]
                self.last_node = _sep_node
                self.further_operation(connect_info)
    ...
    def separation_filt(self, connect_info):
        ...
        for b in _branches:
            ...
            # Next operation of branch
            connect_info=[b]
            if (b=='bout' and _category=='GasFilter') or (b=='tout' and _category=='LiquidFilter'): # by definition we say that the top outlet is the filtrated one, bout directly to OutputProduct
    ...
    def separation_centr(self, connect_info):
        ...
        """
        _current_node = self.last_node
        pattern = random.choices(list(centr_patterns.keys()),
                                 [i [1][0] for i in list(centr_patterns.values())])[0]
        #connect the last node to first of this pattern
        ...
        for b in _branches:
            ...
            # Next operation of branch
            connect_info=[b]
            if b=='bout': # by definition we say that the top outlet is the filtrated one, bout directly to OutputProduct
                outlet_node = 'OutputProduct-%d'%self.operation_counter['OutputProduct']
                self.nodes.append(outlet_node)
                self.edges.append(((_current_node, outlet_node), {'in_connect':connect_info, 'out_connect':[]}))
                self.operation_counter['OutputProduct'] += 1
    ...
    def purification(self, connect_info):
        ...
        if len(list(self.edges))>1 and len(_hexs)>0:
            special_op = random.choices(['Rec', 'HI', 'None'],
                                        [0.15, 0.05, 0.8])[0]
        elif len(list(self.edges))>1:
            special_op = random.choices(['Rec', 'None'],
                                        [0.15, 0.85])[0]
        else:
            special_op='None'
    ...
    def find_edges(self, node, type='incoming'):
    ...
    def adjust_HI_edges(self, old_node_name, new_node_name, in_out):
        ...
        """
        if in_out=='in':
            _incoming = self.find_edges(old_node_name,type='incoming')[0] # only one edge possible
            ...
            _outgoing = self.find_edges(old_node_name,type='outgoing')[0] # only one edge possible
